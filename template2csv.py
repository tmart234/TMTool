...
def getPropData(type):
    prop_str = ''
    # get all property data (all child elements)
    properties = type.find('PropertiesMetaData')
    prop_str = ET.tostring(properties, encoding='utf-8', method='xml')
    # have to dump as json because we have an ordereddicts within ordereddicts
    prop_str = json.loads(json.dumps(dict(xmltodict.parse(prop_str,process_namespaces=True,namespaces=namespaces))))
    return prop_str
    ...
    def __init__(self, root, writer, ele_type):
            for _type in root.findall(ele_type):
                for types in _type.iter('ElementType'):
                    ...
                    # have to dump as json because we have an ordereddicts within ordereddicts
                    self.attribs = json.loads(json.dumps(dict(xmltodict.parse(self.attribs,process_namespaces=True,namespaces=namespaces))))
